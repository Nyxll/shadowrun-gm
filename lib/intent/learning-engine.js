...
export class LearningEngine {
   ...
   */
  async findSimilarPatterns(query) {
    if (!this.db) return [];
    ...
    try {
      ...
      const result = await this.db.query(`
        ...
        WHERE 
          is_active = true
          AND (
            pattern_text ILIKE $1
            OR $2 ILIKE '%' || pattern_text || '%'
            OR pattern_text = ANY(string_to_array($2, ' '))
          )
        ORDER BY 
   ...
   */
  async recordPatternSuccess(patternId, wasSuccessful) {
    if (!this.db) return;
    ...
    try {
      ...
      console.error('Error recording pattern success:', error);
    }
  }
}
