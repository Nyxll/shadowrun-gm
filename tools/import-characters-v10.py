#!/usr/bin/env python3
"""
Import character sheets v10 - FIXES SPELL FORCE PARSING
- All v9 features (proper number parsing, explanations, multi-target modifiers, vehicles, spells)
- FIX: Properly captures and stores learned_force from spell entries
- Format: "**Spell Name Force** (Force Force)" → extracts the force value
- Uses SYSTEM user for audit logging (created_by/modified_by)
"""
import os
import re
...
class CharacterImporterV9:
    """Import characters with proper handling of all formats including spells"""
    ...
    def parse_character_file(self, filepath: str) -> Dict:
        ...
        # Extract magic-specific fields
        magic_pool_match = re.search(r'\*\*Magic Pool\*\*:\s*(\d+)', content)
        if magic_pool_match:
            char_data['magic_pool'] = int(magic_pool_match.group(1))
        else:
            char_data['magic_pool'] = 0
        
        spell_pool_match = re.search(r'\*\*Spell Pool\*\*:\s*(\d+)', content)
        if spell_pool_match:
            char_data['spell_pool'] = int(spell_pool_match.group(1))
        else:
            char_data['spell_pool'] = 0
        
        initiate_match = re.search(r'\*\*Initiate Level\*\*:\s*(\d+)', content)
        if initiate_match:
            char_data['initiate_level'] = int(initiate_match.group(1))
        else:
            char_data['initiate_level'] = 0
        
        # Extract metamagics (comma-separated list)
        metamagics_match = re.search(r'\*\*Metamagics\*\*:\s*([^\n]+)', content)
        if metamagics_match:
            metamagics_text = metamagics_match.group(1).strip()
            if metamagics_text.upper() != 'N/A':
                char_data['metamagics'] = [m.strip() for m in metamagics_text.split(',')]
            else:
                char_data['metamagics'] = []
        else:
            char_data['metamagics'] = []
        
        # Extract magical group/lodge
        magical_group_match = re.search(r'\*\*Magical Group/Lodge\*\*:\s*([^\n]+)', content)
        if magical_group_match:
            char_data['magical_group'] = magical_group_match.group(1).strip()
        else:
            char_data['magical_group'] = None
        
        # Extract tradition and totem
        tradition_match = re.search(r'\*\*Tradition\*\*:\s*([^\n]+)', content)
        if tradition_match:
            tradition_text = tradition_match.group(1).strip()
            char_data['tradition'] = tradition_text
            
            # Extract totem name from parentheses
            # Patterns: "Shamanic (Oak Totem)", "Hermetic", "Shamanic (Bear)"
            totem_match = re.search(r'\(([^)]+?)\s*(?:Totem|Loa)?\)', tradition_text, re.IGNORECASE)
            if totem_match:
                totem_name = totem_match.group(1).strip()
                # Remove "Totem" or "Loa" suffix if present
                totem_name = re.sub(r'\s+(?:Totem|Loa)$', '', totem_name, flags=re.IGNORECASE)
                char_data['totem'] = totem_name
            else:
                char_data['totem'] = None
        else:
            char_data['tradition'] = None
            char_data['totem'] = None
        ...
        # Extract spells with FIXED force parsing
        char_data['spells'] = []
        spells_section = re.search(r'### Spells\s*\n(.*?)(?:\n###|\n---|\n##|$)', content, re.DOTALL | re.IGNORECASE)
        if spells_section:
            for line in spells_section.group(1).split('\n'):
                # Format: - **Spell Name Force** (Force Force)
                # Examples:
                #   - **Treat 6** (Force 6)
                #   - **Stunbolt 5** (Force 5)
                #   - **Improved Invisibility 1** (Force 1)
                
                # Try to match with force in parentheses first
                spell_match = re.match(r'-\s*\*\*([^*]+)\*\*\s*\(Force\s+(\d+)\)', line, re.IGNORECASE)
                if spell_match:
                    full_name = spell_match.group(1).strip()
                    learned_force = int(spell_match.group(2))
                    
                    # Skip N/A and None entries
                    if full_name.upper() in ('N/A', 'NONE'):
                        continue
                    
                    # Extract spell name (remove trailing force number if present)
                    # "Treat 6" → "Treat"
                    # "Stunbolt 5" → "Stunbolt"
                    spell_name = re.sub(r'\s+\d+$', '', full_name).strip()
                    
                    # Auto-categorize based on spell name
                    name_lower = spell_name.lower()
                    if any(x in name_lower for x in ['bolt', 'ball', 'blast', 'lightning', 'powerball', 'stunbolt', 'stunball']):
                        category = 'combat'
                    elif any(x in name_lower for x in ['heal', 'treat', 'stabilize', 'increase', 'decrease']):
                        category = 'health'
                    elif any(x in name_lower for x in ['invisibility', 'mask', 'illusion']):
                        category = 'illusion'
                    elif any(x in name_lower for x in ['levitate', 'barrier', 'control', 'silence']):
                        category = 'manipulation'
                    elif any(x in name_lower for x in ['detect', 'analyze', 'mind probe']):
                        category = 'detection'
                    else:
                        category = 'unknown'
                    
                    spell_type = 'physical' if any(x in name_lower for x in ['physical', 'powerball', 'lightning']) else 'mana'
                    
                    char_data['spells'].append({
                        'spell_name': spell_name,
                        'spell_category': category,
                        'spell_type': spell_type,
                        'learned_force': learned_force  # FIX: Now we capture the force!
                    })
    ...
    def import_character(self, filepath: str):
        ...
        # Insert character with magic fields
        self.cursor.execute("""
        ...
        # Insert spells with learned_force
        for spell in char_data.get('spells', []):
            self.cursor.execute("""
                INSERT INTO character_spells (
                    character_id, spell_name, spell_category, spell_type, 
                    learned_force, drain_modifier
                ) VALUES (%s, %s, %s, %s, %s, %s)
            """, (char_id, spell['spell_name'], spell['spell_category'], 
                  spell['spell_type'], spell.get('learned_force', 1), 0))
