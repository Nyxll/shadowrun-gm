#!/usr/bin/env python3
"""
Import character sheets v9 - ADDS SPELL SUPPORT
- All v8 fixes (proper number parsing, explanations, multi-target modifiers, vehicles)
- NEW: Parses and imports character spells from ### Spells section
- Auto-categorizes spells (combat, health, illusion, manipulation, detection)
- Determines spell type (mana vs physical)
"""
import os
import re
    ...
    def parse_character_file(self, filepath: str) -> Dict:
        ...
        if spells_section:
            for line in spells_section.group(1).split('\n'):
                spell_match = re.match(r'-\s*\*\*([^*]+)\*\*', line, re.IGNORECASE)
                if spell_match:
                    spell_name = spell_match.group(1).strip()
                    
                    # Skip N/A and None entries
                    if spell_name.upper() in ('N/A', 'NONE'):
                        continue
                    
                    # Remove "Force X" suffix if present
                    spell_name = re.sub(r'\s+Force\s+\d+$', '', spell_name, flags=re.IGNORECASE)
                    spell_name = re.sub(r'\s+\d+$', '', spell_name)
        ...
        for spell in char_data.get('spells', []):
            self.cursor.execute("""
                INSERT INTO character_spells (
                    character_id, spell_name, spell_category, spell_type, drain_modifier
                ) VALUES (%s, %s, %s, %s, %s)
            """, (char_id, spell['spell_name'], spell['spell_category'], 
                  spell['spell_type'], 0))
        
        if char_data.get('spells'):
            print(f"  ✓ Added {len(char_data['spells'])} spells")
