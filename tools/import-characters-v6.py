#!/usr/bin/env python3
"""
Import character sheets v5 - Enhanced parser with SPIRITS support
Handles: base (modified) (explanation) format for attributes and skills
Adds: Power points tracking for Physical/Mystical Adepts
Adds: Complete cyberware/bioware modifier parsing (Math SPU, Encephalon, etc.)
Adds: Bound spirits parsing and storage
"""
import os
import re
...
class CharacterImporterV5:
    """Import characters with enhanced modifier parsing and SPIRITS support"""
    ...
    def parse_stat_with_modifiers(self, stat_line: str) -> Tuple[int, Optional[int], List[str]]:
        """
        Parse new format: base (modified) (explanation)
        Example: "6 (10) (+4 from Quickened Increase Attribute +4 force 3)"
        
        Returns: (base_value, modified_value, explanations)
        """
        # Pattern: number (optional: (number)) (optional: (explanation))
        # First number is ALWAYS base
        match = re.match(r'^\s*(\d+)\s*(?:\((\d+)\))?\s*(?:\(([^)]+)\))?\s*$', stat_line.strip())
        
        if not match:
            # Fallback: just a number
            num_match = re.match(r'^\s*(\d+)\s*$', stat_line.strip())
            if num_match:
        ...
        modified = int(match.group(2)) if match.group(2) else None
        explanation = match.group(3) if match.group(3) else None
        
        explanations = []
        if explanation:
            # Parse explanation for individual modifiers
            # Example: "+4 from Quickened Increase Attribute +4 force 3"
            # or "+1 Supra-Thyroid Gland +4 Muscle Augmentation"
            explanations = [exp.strip() for exp in explanation.split(',')]
        
        return (base, modified, explanations)
    ...
    def parse_modifier_line(self, line: str) -> Optional[List[Dict]]:
        """
        Parse a modifier line from bioware/cyberware description
        Returns list of modifier dicts or None
        Each dict includes 'description' field with original text
        """
        ...
        # Check for modifiers in parentheses (e.g., "Enhanced with Project Aegis software (-3 TN)")
        paren_match = re.search(r'\(([^)]+)\)', line)
        if paren_match:
            # Try to parse what's in parentheses
            paren_content = paren_match.group(1)
            result = self.parse_modifier_line(paren_content)
            if result:
                return result
        ...
        if phys_skill_match:
            ...
            return [{
                ...
                'modifier_value': value,
                'description': original_line
            } for skill in ['firearms', 'whips', 'monowhip', 'gunnery', 'athletics', 'stealth']]
        ...
        enhanced_match = re.search(r'Enhanced\s+with.*\(([^)]+)\)', line, re.IGNORECASE)
        if enhanced_match:
            ...
            for item in items[1:]:
                ...
                if name_match:
                    ...
                    for line in lines[1:]:
                        line = line.strip()
                        if line.startswith('-'):
                            mods = self.parse_modifier_line(line[1:].strip())
                            if mods:
        ...
        if attr_section:
            for line in attr_section.group(1).split('\n'):
                ...
                if match:
                    ...
                    base, modified, explanations = self.parse_stat_with_modifiers(stat_text)
                    ...
                    # Extract modifiers from explanations
                    if explanations:
                        mods = self.extract_modifiers_from_explanation(explanations, attr_name)
                        char_data['attribute_modifiers'].extend(mods)
        ...
        if skills_section:
            ...
            for line in skills_section.group(1).split('\n'):
                ...
                if match:
                    ...
                    # Extract skill modifiers from explanations
                    for exp in explanations:
